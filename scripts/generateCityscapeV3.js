// Generate cityscape V3 - Remove horn honks, add more trains
// Run with: node scripts/generateCityscapeV3.js
...
function generateCityscapeV3() {
  ...
  for (let ch = 0; ch < 2; ch++) {
    ...
    // REMOVED: Horn honks (user said they're weird)
    
    // MORE TRAINS: Continuous distant train rumble (louder and more present)
    for (let i = 0; i < length; i++) {
      const t = i / SAMPLE_RATE;
      // Low frequency rhythmic rumble (train on tracks)
      const clatterFreq = 2.2 + Math.sin(t * 0.2) * 0.4; // 1.8-2.6 Hz rhythm
      const clatter = Math.sin(2 * Math.PI * clatterFreq * t);
      // Make it louder and add harmonics
      const trainRumble = clatter * 0.035 + Math.sin(2 * Math.PI * clatterFreq * 2 * t) * 0.015;
      data[i] += trainRumble;
    }
    
    // Train horns - MORE FREQUENT (every 12-20 seconds instead of 20-35)
    ...
    while (trainTime < DURATION) {
      ...
      for (let i = 0; i < trainLength && trainStart + i < length; i++) {
        ...
        const freq2 = 329.63; // High tone
        
        let envelope;
        if (t < 0.1) {
          envelope = t / 0.1 * 0.08; // LOUDER (was 0.06)
        } else if (t > 1.2) {
      ...
      trainTime += 12 + Math.random() * 8; // More frequent
    }
    ...
    while (bellTime < DURATION) {
      ...
      const bellDuration = 4.0; // 4 seconds of crossing bell
      const bellLength = Math.floor(bellDuration * SAMPLE_RATE);
      const bellFreq = 800; // Classic crossing bell pitch
      
      for (let i = 0; i < bellLength && bellStart + i < length; i++) {
        const t = i / SAMPLE_RATE;
        // Two-beat pattern: ding-ding (pause) ding-ding
        const beatCycle = (t % 1.5); // 1.5 second cycle
        let beatEnvelope = 0;
    ...
    while (screechTime < DURATION) {
      ...
      for (let i = 0; i < screechLength && screechStart + i < length; i++) {
        const t = i / SAMPLE_RATE;
        const progress = t / screechDuration;
        
        // Metallic screech sound
        const freq = 800 + Math.sin(t * 30) * 200; // Wavering frequency
    ...
    // Construction sounds (occasional)
    ...
    while (constructionTime < DURATION) {
      ...
      for (let i = 0; i < conLength && conStart + i < length; i++) {
        ...
        const rhythm = Math.sin(2 * Math.PI * 3 * t);
  ...
  writeWavFile('cityscape_v3.wav', channels);
}

console.log('Generating cityscape V3 (more trains, no horns)...\n');
generateCityscapeV3();
console.log('\n✓ Cityscape V3 generated!');
console.log('Changes from V2:');
console.log('- ✗ Removed random horn honks (too weird)');
console.log('- ✓ Louder continuous train rumble (0.035 vs 0.015)');
console.log('- ✓ More frequent train horns: every 12-20s (was 20-35s)');
console.log('- ✓ Louder train horns: 0.08 (was 0.06)');
console.log('- ✓ Added train crossing bells: every 25-40s (ding-ding pattern)');
console.log('- ✓ Added train brake screeches: every 30-50s');
console.log('- ✓ Train rumble has harmonics for richer sound');
console.log('\nKeeping: Cars, trucks, people chatter, construction');
