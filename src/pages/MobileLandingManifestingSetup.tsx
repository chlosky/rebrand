...
function MobileLandingManifestingSetupPage() {
  const { t } = useMarketingTranslation();
  ...
  useEffect(() => {
    ...
    trackMarketingConversion("landing_view", {
      page_path: pagePath,
      content_id: pagePath,
      content_name: "mobile_landing_manifesting_setup",
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col pb-0">
          ...
          <div className="mx-auto flex max-w-7xl items-center px-4 py-3 sm:px-6 sm:py-4">
            <span
              className={cn(
                ...
                "whitespace-nowrap text-[clamp(13px,3.4vw+0.55rem,18px)] leading-tight",
              )}
            >
              The Manifesting Setup x Timeline Shifter
            </span>
          </div>
        </header>
        ...
        >
          <div className="w-fit max-w-full text-left">
            <span className={cn(MARKETING_LOGO_CLASS, "mb-2 block text-lg")}>Palette Plotting</span>
            <MarketingHeroCopy
              compact
          ...
          <div className="mt-6 w-full max-w-[min(100%,23rem)] self-start">
            <MarketingStoreBadges
              ...
              onStoreClick={(store) => cta.onStoreClick("mobile_landing_manifesting_setup_store", store)}
            />
          </div>
...
export default function MobileLandingManifestingSetup() {
  return (
    <MarketingStoreCtaProvider>
      <MobileLandingManifestingSetupPage />
    </MarketingStoreCtaProvider>
  );
}
