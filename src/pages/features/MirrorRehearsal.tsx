...
import { getLocalizedPremadeSets, getSupportCategoryLabel } from "@/lib/affirmations-data";
...
/** iOS native + web/PWA Mirror Work. Android uses MirrorRehearsalAndroid via MirrorRehearsalRoute. */
const MirrorRehearsal: React.FC = () => {
...
const { startNativeCamera } = useNativeCamera();
...
const isNativeIosMirror = Capacitor.isNativePlatform() && Capacitor.getPlatform() === "ios";
...
const nativeSceneActive = isNativeIosMirror && active && ready && nativeScene !== "none";
...
ictx.drawImage(video, sx, sy, sw, sh, 0, 0, targetW, targetH);
  ...
  try {
    const numBefore = tf.engine().state.numTensors;
    ...
    const segmentation = await net.segmentPerson(input, {
      ...
      segmentationThreshold: isMobile ? 0.5 : 0.7,
    });

    // Dispose any leaked tensors from this segmentation pass
    const numAfter = tf.engine().state.numTensors;
    if (numAfter - numBefore > 20) {
      tf.disposeVariables();
    }
...
// Real-time volume detection (disabled on iOS native — same as Android)
useEffect(() => {
...
// Native iOS durable compositor path (Vision/CoreImage) mounted over mirror area.
useEffect(() => {
  if (!isNativeIosMirror) return;
  ...
  const startNative = async () => {
    ...
    try {
      ...
      console.error("[NativeMirror] start failed:", err);
    }
  };
  ...
  // Scroll can move the mirror card relative to the viewport; keep native frame pinned.
  ...
  // iOS can shift viewport during momentum scroll; RAF sync keeps the native view in lockstep.
...
}, [isNativeIosMirror, active, ready, nativeScene]);
...
ensureMeta("description", t("mirror.metaDescription"));
...
setError(isNativePerm
  ? i18n.t("mirror.errors.permissionDeniedNative", { ns: "tools" })
  : i18n.t("mirror.errors.permissionDeniedSafari", { ns: "tools" }));
return;
}
} catch (e) {
...
// Try native camera hook first (only works on native iOS)
try {
stream = await startNativeCamera(videoRef.current, constraints);
} catch (nativeError) {
console.log("[Mirror] Native camera not available, using web API:", nativeError);
}

// Fall back to web API if native didn't work (or on web/PWA)
if (!stream) {
...
if (!stream) {
setError(i18n.t("mirror.errors.unableAccess", { ns: "tools" }));
return;
}
...
message = Capacitor.isNativePlatform()
  ? i18n.t("mirror.errors.permissionDeniedIosNative", { ns: "tools" })
  : i18n.t("mirror.errors.permissionDeniedIosSafari", { ns: "tools" });
} else {
...
opacity: (() => {
  ...
  // On native, keep video visible so drawImage(video) gets live frames (iOS can fail otherwise)
  const usingCanvasScene = 
    selectedPack === "hearts-overlay" ||
    selectedPack === "coins-overlay" ||
    selectedPack === "gold-sparks-overlay" ||
  ...
  if (usingCanvasScene) {
    if (Capacitor.isNativePlatform()) return 1; // Native web-canvas mode: keep video paintable for canvas sampling
    return canvasHasDrawn ? 0 : 1; // Web: hide after first canvas draw
  }
  return 1;
})(),
imageRendering: 'auto',
...
{Capacitor.isNativePlatform()
  ? t("mirror.permissionHints.native")
...
{active && showMeter && hasMeterAccess && !isNativeIosMirror && (
...
<div className={cn("grid gap-4 sm:gap-6 items-start", isNativeIosMirror ? "grid-cols-2" : "grid-cols-3")}>
{/* Affirmation Set Selector */}
<div className="flex flex-col gap-2">
<label className="text-sm font-medium text-black whitespace-nowrap">{t("mirror.affirmationSet")}</label>
<Select
                  value={selectedSetId ?? ""}
...
<SelectValue placeholder={t("mirror.selectPlaceholder")} className="text-black text-left" />
</SelectTrigger>
<SelectContent className="bg-white z-50 border-border text-black">
{affirmationSets.length === 0 ? (
<SelectItem value="none" disabled className="text-black">{t("mirror.noSetsAvailable")}</SelectItem>
) : (
<>
...
if (hasMeterAccess) setShowMeter(!!checked);
}}
...
<label className="text-sm font-medium text-black">{t("mirror.displaySpeed")}</label>
...
export default MirrorRehearsal;