...
export type ManifestationMilestonesTabsProps = {
  /** When false (e.g. Your Journey), tab state is local only — no `location.hash` reads/writes. */
  syncHash?: boolean;
};

export function ManifestationMilestonesTabs({ syncHash = true }: ManifestationMilestonesTabsProps) {
  const { t, i18n } = useTranslation("tools");
  const { user } = useAuth();
  const { theme } = useTheme();
  const { activePractices } = useEmbodyActivePractices();
  const dateLocale = useMemo(
    () => (i18n.language === "pt-BR" ? "pt-BR" : i18n.language.startsWith("es") ? "es" : "en-US"),
    [i18n.language],
  );
  const compactWeekdayDates =
    i18n.language === "pt-BR" || i18n.language.startsWith("es");
              ...
              <PopoverTrigger asChild>
                <Button
                  ...
                  style={
                    selectedCategory
                      ? {
                          borderColor: SUPPORT_CATEGORIES.find((c) => c.name === selectedCategory)?.color,
                          backgroundColor: `${SUPPORT_CATEGORIES.find((c) => c.name === selectedCategory)?.color}20`,
                          color: SUPPORT_CATEGORIES.find((c) => c.name === selectedCategory)?.color,
                        }
                      : undefined
                  }
                >
                  {selectedCategory ? getSupportCategoryLabel(selectedCategory) : t("activity.milestones.goals.category")}
              ...
              <PopoverContent className="p-2" align="start" style={categoryButtonWidth ? { width: `${categoryButtonWidth}px` } : {}}>
                <div className="max-h-64 overflow-y-auto space-y-2">
                  {SUPPORT_CATEGORIES.map((category) => (
                    ...
                    >
                      {getSupportCategoryLabel(category.name)}
                    </button>
                  ))}
                </div>
              </PopoverContent>
          ...
          <div className="space-y-2">
            {isLoadingGoals ? (
              ...
              weeklyGoals.map((goal) => {
                const categoryInfo = goal.category ? SUPPORT_CATEGORIES.find((c) => c.name === goal.category) : null;
                  ...
                  >
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      {categoryInfo && (
                        ...
                        >
                          {getSupportCategoryLabel(categoryInfo.name)}
                        </span>
                      )}
                    </div>
                    <Button
                ...
                <div className={cn(milestoneInnerSurfaceClass, "p-4")}>
                  ...
                  <div className="space-y-3">
                    {Object.entries(weekStats.categoryBreakdown)
                      ...
                      .map(([category, stats]) => {
                        const categoryInfo = SUPPORT_CATEGORIES.find((c) => c.name === category);
