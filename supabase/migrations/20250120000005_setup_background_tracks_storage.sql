-- Setup storage bucket policies for background-tracks
...
-- ============================================================================
-- Drop existing policies for background-tracks bucket (if any)
-- ============================================================================
DROP POLICY IF EXISTS "Users can upload their own background tracks" ON storage.objects;
DROP POLICY IF EXISTS "Users can view their own background tracks" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their own background tracks" ON storage.objects;
DROP POLICY IF EXISTS "Users can update their own background tracks" ON storage.objects;
DROP POLICY IF EXISTS "Public can view background tracks" ON storage.objects;
...
-- ============================================================================
CREATE POLICY "Users can upload their own background tracks"
  ON storage.objects FOR INSERT
  TO authenticated
  WITH CHECK (
    bucket_id = 'background-tracks' AND
