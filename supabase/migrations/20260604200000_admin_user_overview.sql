-- Admin ops snapshot table. Does not ALTER any existing tables.
-- Populated only by refresh_admin_user_overview() (read sources, write this table).
-- Requires manifestation routine columns on user_preferences/profiles (20260604210000).
...
CREATE TABLE public.admin_user_overview (
  ...
  user_plans_google_play_customer_id text,
...
BEGIN
  ...
  INSERT INTO public.admin_user_overview (
    ...
    user_plans_google_play_customer_id,
...
-- Studio reads charge rows directly (snapshot does not denormalize charge).
