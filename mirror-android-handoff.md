# Mirror Work — Android full code handoff

**Project:** Palette Plotting / belief-craft-nexus  
**Branch:** `Mobile-app`  
**Route:** `/dashboard/mirror` → `MirrorRehearsalRoute` → `MirrorRehearsalAndroid` (Android native only)  
**Stack:** React + Capacitor 8 + CameraX + ML Kit + BodyPix canvas fallback + Web Audio volume meter

## How Android is selected
...
3. `src/pages/features/MirrorRehearsalAndroid.tsx`
4. `android/app/src/main/java/com/paletteplotting/app/NativeMirrorPlugin.java`
5. `android/app/src/main/java/com/paletteplotting/app/NativeMirrorView.java`
...
// Anchor the WebView audio session to Android's USAGE_MEDIA stream so that
// the hardware volume rocker controls Mirror's WebAudio-rendered scene
// sounds (rain, hearts, coins, gold sparks, summit wind). Without an
// HTMLAudioElement actively producing media, Android WebView often routes
// WebAudio to a non-media stream that the volume keys do not adjust.
// A single tiny silent loop is enough to claim the media audio focus.
