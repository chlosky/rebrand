# Mirror Work — iOS + Web full code handoff (Part 2 of 2)

**Main component:** `src/pages/features/MirrorRehearsal.tsx`  
**Part 1 (route, plugin bridge, camera hook, overlay bootstrap, iOS Swift):** mirror-ios-web-handoff.md
...
message = Capacitor.isNativePlatform()
  ? "Camera permission denied. Tap 'Allow' when prompted, or go to Settings > Palette Plotting > Camera."
  : "Camera permission denied. Tap 'Allow' when prompted, or go to Settings > Safari > Camera and enable it for this site.";
} else {
...
checked={showMeter && hasMeterAccess && !isNativeIosMirror}
onCheckedChange={(checked) => {
if (hasMeterAccess && !isNativeIosMirror) {
setShowMeter(!!checked);
}
}}
disabled={!hasMeterAccess || isNativeIosMirror}
className={cn(
...
(!hasMeterAccess || isNativeIosMirror) && "opacity-50 cursor-not-allowed"
)}
/>
<label
